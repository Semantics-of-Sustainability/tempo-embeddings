...
class WeaviateDatabaseManager(VectorDatabaseManagerWrapper):
        ...
        *,
        ...
        if self.embedder_config is None:
            if os.path.exists(self.db_path):
                ...
                raise FileNotFoundError(
                    "No existing configuration was found for this Database. Did you provide the right path?"
                )
        elif self.embedder_config.get("type") == "hf":
            self.embedding_function = (
                wvc.config.Configure.Vectorizer.text2vec_huggingface(
                    model=self.embedder_name
                )
            )
            weaviate_headers["X-HuggingFace-Api-Key"] = self.embedder_config["api_key"]
        elif (
            self.embedder_config.get("type") == "custom_model"
            or self.embedder_config.get("type") == "default"
        ):
            try:
                ...
                logger.error(
                    "If the type is 'custom_model' or 'default' you should pass the model object under Key 'model': %s",
                    str(e),
                )
        else:
            raise ValueError(
                f"Malformed embedder_config {self.embedder_config}. Check that 'type', 'api_key' and 'model' keys are properly populated."
            )
        ...
        if not self.connect():
            raise ConnectionError(
                f"Could not connect to Weaviate database at host: {self._client.get_meta()['hostname']}"
            )
    ...
    def delete_collection(self, name):
        try:
            ...
            logger.error(
                "delete_collection() with name '%s' caused exception %s", name, e
            )
    ...
    def insert_corpus(self, collection: str, corpus: Corpus):
        ...
        if self.model:
            ...
            raise ValueError(
                "There is no valid way to vectorize the texts. Provide a custom model or a valid weaviate embedder"
            )
