...
class Corpus:
    ...
    def set_topic_label(
        self, vectorizer: TfidfVectorizer, exclude_word: str = "", n: int = 1
    ) -> None:
        ...
        if self.label != Corpus.OUTLIERS_LABEL:
            # rewrite cluster labels
            top_words: list[str] = [
                word
                for word in self.topic_words(vectorizer, n + 1)
    ...
    def topic_words(self, vectorizer: TfidfVectorizer, n: int = 5) -> list[str]:
        ...
        Args:
            vectorizer: The vectorizer to use for tf-idf scoring;
                typically generated by calling Corpus.tfidf_vectorizer() on a super-corpus.
