...
class AbstractCorpus(ABC):
    ...
    def _tf_idf_words(self, vectorizer: TfidfVectorizer, n: int = None) -> list[str]:
        ...
        assert tf_idfs.shape == (
            len(self.passages),
            len(AbstractCorpus.get_vocabulary(vectorizer)),
        ...
        assert weighted_tf_idfs.shape[0] == len(
            AbstractCorpus.get_vocabulary(vectorizer)
        )

        # pylint: disable=invalid-unary-operand-type
        top_indices = np.argsort(-weighted_tf_idfs)[:n]

        return [AbstractCorpus.get_vocabulary(vectorizer)[i] for i in top_indices]
